...
public:
    ...
    const Matrix22 &getCentroidCov() const {
        prepare();
...
public:
    ...
    const PMCovariance &getInvCovPM() const {
        prepare();
        return pmFisher;
    }  //??
    // Return expected covariance matrix of PM, after weight solution.
    const PMCovariance &getTrueCovPM() const {
        prepare();
        return pmTrueCov;
    }
...
public:
    ...
    int sigmaClip(double sigThresh, bool doReserved = false, bool clipEntireMatch = false,
                  bool logging = true);
    ...
    void operator()(const DVector &params, double &chisq, DVector &beta, DMatrix &alpha,
                    bool reuseAlpha = false);
