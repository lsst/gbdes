...
void Detection::buildProjector(double pmTDB,         // Time in years from PM reference epoch
    ...
    ReferenceFrame frame(CartesianICRS(0., 0., 0.), Orientation(icrs));
...
Vector2 Detection::residWorld() const {
    Vector2 out;
    Assert(itsMatch);
    itsMatch->solve();
    out[0] = xw;
...
Vector2 Detection::residPix() const {
    Assert(itsMatch);
    itsMatch->solve();
    ...
    Assert(map);
    try {
        map->toPix(xyMean[0], xyMean[1], xMeanPix, yMeanPix, color);
...
double PMDetection::trueChisq() const {
...
bool Match::isFit(const Detection &e) {
...
void Match::prepare() const {
    ...
    if (dof < 0) {
        ...
        for (auto const &i : elist) i->expectedTrueChisq = 0.;
        centroidF.setZero();
    ...
    for (auto const &i : elist) {
        if (i->invCov(0, 0) <= 0.) {
            ...
            i->expectedTrueChisq =
                    2 - (2 * i->fitWeight) * traceATB(i->invCov, invF) + traceATB(i->invCov, trueCentroidCov);
        }
    }
...
void Match::remap(bool doAll) const {
...
int Match::accumulateChisq(double &chisq, DVector &beta, SymmetricUpdater &updater, bool reuseAlpha) {
    int nP = beta.size();
    ...
    for (auto i = elist.begin(); i != elist.end(); ++i, ++ipt) {
        ...
        if (npi > 0) {
            ...
            (*i)->map->toWorldDerivs((*i)->xpix, (*i)->ypix, (*i)->xw, (*i)->yw, *dXYdP[ipt], (*i)->color);
        } else if (!isMappedFit) {
        ...
        for (int iMap = 0; iMap < (*i)->map->nMaps(); iMap++) {
            ...
            if (!reuseAlpha) {
                ...
                for (int iMap2 = iMap + 1; iMap2 < (*i)->map->nMaps(); iMap2++) {
                    ...
                    updater.update(mapNumber2, ip2, dxy2, invCovW, mapNumber, ip, dxy1);
                    istart2 += np2;
                }
            }
            istart += np;
    ...
    if (!reuseAlpha) {
        ...
        */
        ...
        for (auto m1 = mapsTouched.begin(); m1 != mapsTouched.end(); ++m1) {
            int map1 = m1->first;
            ...
            for (; m2 != mapsTouched.end(); ++m2) {
                ...
                updater.update(map2, i2, dxy2, negCovCentroid, map1, i1, dxy1);
            }
        }
...
bool Match::sigmaClip(double sigThresh, bool deleteDetection) {
    ...
    if (worst != elist.end()) {
#ifdef DEBUG
        cerr << "clipped " << (*worst)->catalogNumber << " / " << (*worst)->objectNumber << " at "
             << (*worst)->xw << "," << (*worst)->yw << " resid " << setw(6)
             << ((*worst)->xw - xyMean[0]) * WCS_UNIT / RESIDUAL_UNIT;
        << " " << setw(6) << ((*worst)->yw - xyMean[1]) * *WCS_UNIT / RESIDUAL_UNIT;
        << " resid/sig " << np.sqrt(maxSq) << endl;
#endif
        if (deleteDetection) {
            elist.erase(worst);
        } else {
...
double Match::chisq(int &dofAccum, double &maxDeviateSq, bool dump) const {
    solve();
    double chi = 0.;
        ...
        if (auto ii = dynamic_cast<PMDetection *>(i.get())) {
            ...
            if (i->invCov(0, 0) <= 0.) {
                ...
                if (i->fitWeight == 0.) {
                    ...
                    i->expectedTrueChisq =
                            2. - 2 * i->fitWeight * traceATB(tmp, pmInvFisher) + traceATB(tmp, pmTrueCov);
                }
            }
        }
    }
...
double PMMatch::chisq(int &dofAccum, double &maxDeviateSq, bool dump) const {
...
bool PMMatch::sigmaClip(double sigThresh, bool deleteDetection) {
    ...
    if (worst != elist.end()) {
        ...
        if (auto ii = dynamic_cast<const PMDetection *>((*worst)->get())) {
            cerr << "clipped PM " << (*worst)->catalogNumber << " / " << (*worst)->objectNumber << " at "
                 << (*worst)->xw << "," << (*worst)->yw << " resid/sig " << sqrt(maxSq) << endl;
        } else {
            cerr << "clipped " << (*worst)->catalogNumber << " / " << (*worst)->objectNumber << " at "
                 << (*worst)->xw << "," << (*worst)->yw << " resid " << setw(6)
                 << ((*worst)->xw - predict(worst->get())[0]) * WCS_UNIT / RESIDUAL_UNIT << " " << setw(6)
                 << ((*worst)->yw - predict(worst)[1]) * WCS_UNIT / RESIDUAL_UNIT << " resid/sig "
                 << sqrt(maxSq) << endl;
        }
#endif
        if (deleteDetection) {
...
Vector2 PMMatch::predict(const Detection *d) const {
...
int PMMatch::accumulateChisq(double &chisq, DVector &beta, SymmetricUpdater &updater, bool reuseAlpha) {
    int nP = beta.size();
            ...
            for (int iMap = 0; iMap < (*i)->map->nMaps(); iMap++) {
                ...
                if (!reuseAlpha) {
                    ...
                    for (int iMap2 = iMap + 1; iMap2 < (*i)->map->nMaps(); iMap2++) {
                        ...
                        if (np2 == 0) continue;
#ifdef USE_TMV
                        tmv::ConstVectorView<double> dxy2 =
                                dXYdP[ipt]->subMatrix(0, 2, istart2, istart2 + np2);
#elif defined USE_EIGEN
                        DMatrix dxy2 = dXYdP[ipt]->block(0, istart2, 2, np2);
#endif
                        // Now update below diagonal
        ...
        */
        ...
        for (auto m1 = mapsTouched.begin(); m1 != mapsTouched.end(); ++m1) {
            ...
            for (; m2 != mapsTouched.end(); ++m2) {
                ...
                updater.update(map2, i2, dxy2, negCov, map1, i1, dxy1);
            }
        }  // Finished putting terms from mean into alpha
    }
...
/////////////////////////////////////////////////////////////////////
void CoordAlign::setParams(const DVector &p) {
    // First, change parameters in the map collection
    pmc.setParams(p);
    // Then alert all Matches that their world coordinates are crap
    for (auto const &m : mlist) m->mapsHaveChanged();
}

void CoordAlign::operator()(const DVector &p, double &chisq, DVector &beta, DMatrix &alpha, bool reuseAlpha) {
    int nP = pmc.nParams();
    Assert(p.size() == nP);
    Assert(beta.size() == nP);
            ...
            if (blank) {
                ...
                if (frozenParameters.count(i) > 0) {
                    ...
                    FormatAndThrow<AstrometryError>()
                            << "Frozen parameter " << i << " in map " << badAtom << " became constrained??";
                }
            }
        }  // End alpha row loop
...

    {
        ...
        if (precondition) {
            for (int i = 0; i < N; i++) {
                ...
                for (int j = 0; j <= i; j++) alpha(i, j) *= ss(i);
                for (int j = i; j < N; j++) alpha(j, i) *= ss(i);
            }
        }
        ...
        if (inPlace) {
            ...
            if (inplaceLLT->info() == Eigen::NumericalIssue) choleskyFails = true;
        } else {
            llt = new LLT(alpha);
...
int CoordAlign::sigmaClip(double sigThresh, bool doReserved, bool clipEntireMatch, bool logging) {
    int nclip = 0;
...
double CoordAlign::chisqDOF(int &dof, double &maxDeviate, bool doReserved) const {
    dof = 0;
    maxDeviate = 0.;
    double chisq = 0.;
...
void CoordAlign::count(long int &mcount, long int &dcount, bool doReserved, int minMatches) const {
    mcount = 0;
    dcount = 0;
    for (auto const &i : mlist) {
        if ((i->getReserved() ^ doReserved) || i->getDOF() < 0 || i->fitSize() < minMatches) {
            continue;
        }
        mcount++;
        dcount += i->fitSize();
...
void CoordAlign::count(long int &mcount, long int &dcount, bool doReserved, int minMatches,
                       long catalog) const {
    mcount = 0;
    dcount = 0;
    for (auto const &i : mlist) {
        if ((i->getReserved() ^ doReserved) || i->getDOF() < 0 || i->fitSize() < minMatches) continue;

        int ddcount = 0;
        for (auto const &d : *i) {
            if (!(d->isClipped) && d->catalogNumber == catalog) ddcount++;
        }
