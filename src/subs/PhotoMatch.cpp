...
double Detection::residMag() const {
    Assert(itsMatch);
    itsMatch->solve();
...
void Match::prepare() const {
    ...
    for (auto const &i : elist) {
        if (i->invVar <= 0.) {
            ...
            i->expectedTrueChisq = 1 - 2. * i->fitWeight * i->invVar * invF + i->invVar * trueMeanVar;
        }
    }
...
int Match::accumulateChisq(double &chisq, DVector &beta, SymmetricUpdater &updater, bool reuseAlpha) {
    ...
    for (auto i = elist.begin(); i != elist.end(); ++i, ++ipt) {
        ...
        if (npi > 0) {
            ...
            (*i)->magOut = (*i)->map->forwardDerivs((*i)->magIn, (*i)->args, *di[ipt]);
        } else if (!isMappedFit) {
        ...
        for (int iMap = 0; iMap < (*i)->map->nMaps(); iMap++) {
            ...
            if (!reuseAlpha) {
                ...
                for (int iMap2 = iMap + 1; iMap2 < (*i)->map->nMaps(); iMap2++) {
                    ...
                    updater.rankOneUpdate(mapNumber2, ip2, dm2, mapNumber, ip, dm1, invVarW);
                    istart2 += np2;
                }
                istart += np;
            }
        ...
        */
        ...
        for (auto m1 = mapsTouched.begin(); m1 != mapsTouched.end(); ++m1) {
            ...
            for (; m2 != mapsTouched.end(); ++m2) {
                ...
                updater.rankOneUpdate(map2, i2, dm2, map1, i1, dm1, negVarMean);
            }
        }
    }
    return dof;
...
bool Match::sigmaClip(double sigThresh, bool deleteDetection) {
    ...
    if (worst != elist.end()) {
#ifdef DEBUG
        cerr << "clipped " << worst->catalogNumber << " / " << worst->objectNumber << " at " << worst->magOut
             << " resid " << setw(6) << (worst->magOut - meanMag) << " sigma " << 1. / sqrt(worst->invVar)
             << endl;
#endif
        if (deleteDetection) {
            elist.erase(worst);
...
double Match::chisq(int &dofAccum, double &maxDeviateSq) const {
    solve();
    double chi = 0.;
    if (dof <= 0) return chi;
    for (auto const &i : elist) {
...
void PhotoAlign::setParams(const DVector &p) {
...
DVector PhotoAlign::getParams() const {
...
void PhotoAlign::operator()(const DVector &p, double &chisq, DVector &beta, DMatrix &alpha, bool reuseAlpha) {
    countPriorParams();
    int nP = nParams();
    Assert(p.size() == nP);
    Assert(beta.size() == nP);
...
#pragma omp critical(beta)
    ...
    for (auto i : priors) i->accumulateChisq(chisq, beta, updater, reuseAlpha);
            ...
            if (blank) {
                ...
                if (i < pmc.nParams()) {
                    ...
                    for (auto iprior : priors) {
                        if (i >= iprior->startIndex() && i < iprior->startIndex() + iprior->nParams()) {
                ...
                if (frozenParameters.count(i) > 0) {
                    if (i < pmc.nParams()) {
                        ...
                        FormatAndThrow<PhotometryError>() << "Frozen parameter " << i << " in map " << badAtom
                                                          << " became constrained??";
                    } else {
                        FormatAndThrow<PhotometryError>()
                                << "Frozen parameter " << i << " in prior became constrained??";
                    }
                }
            }
        }  // End alpha row loop
...
double PhotoAlign::fitOnce(bool reportToCerr, bool inPlace) {
    DVector p = getParams();
...
int PhotoAlign::sigmaClip(double sigThresh, bool doReserved, bool clipEntireMatch, bool logging) {
    int nclip = 0;
...
int PhotoAlign::sigmaClipPrior(double sigThresh, bool clipEntirePrior) {
    int nClip = 0;
    for (auto i : priors)
        if (i->sigmaClip(sigThresh)) {
            // Clipped something.
...
double PhotoAlign::chisqDOF(int &dof, double &maxDeviate, bool doReserved) const {
    dof = 0;
    maxDeviate = 0.;
    double chisq = 0.;
...
#pragma omp parallel for schedule(dynamic, chunk) reduction(+ : dof) reduction(+ : chisq)
    ...
    if (!doReserved) {
        ...
        dof -= pmc.nParams();
        for (auto i : priors) chisq += i->chisq(dof);
    }
    return chisq;
}

void PhotoAlign::count(long int &mcount, long int &dcount, bool doReserved, int minMatches) const {
    mcount = 0;
    dcount = 0;
    for (auto const &i : mlist) {
...
void PhotoAlign::count(long int &mcount, long int &dcount, bool doReserved, int minMatches,
                       long catalog) const {
    mcount = 0;
    dcount = 0;
    for (auto const &i : mlist) {
...
void PhotoAlign::countPriorParams() {
